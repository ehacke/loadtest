...
function HttpClient(operation, params)
{
  ...
  var sessionId = 0;
    ...
    // Handle host/port settings differently if a proxy is specified
    if (params.proxy) {
      var proxy = urlLib.parse(params.proxy);
    ...
    if (params.headers)
    {
      ...
      options.headers['proxy-session-id'] = 'session' + sessionId++;

      if (params.proxy && params.proxyUser) {
        var username = params.proxyUser.split(':')[0];
        var password = params.proxyUser.split(':')[1];

        options.headers['proxy-authorization'] = 'Basic ' + btoa(username + ':' + password);
      }
    }
 ...
 *  - proxy: host:ip of proxy server
 *  - proxyUser: user:password for proxy server authentication
